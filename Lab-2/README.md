...
*Откроем  CLI конфигурации коммутаторов S1 и S2*
   ...
   1. Настроим соответственно их IP-адреса, как указано в таблице адресации.
...
-___Да, мы снова можем наблюдать мас-адрес свитча S1 во VLAN 1 на интерфейсе Fa0/1  по истечении некоторого времени, поскольку интерфейсы свитчей могут обмениваться служебной информацией и находятся в режиме изучения портов.___
 
### 4. **С компьютера PC-B отправим эхо-запросы устройствам в сети и просмотрите таблицу МАС-адресов коммутатора.**

Выполним в командной строке компьютера последовательно эхо=запросы с соответствующими IP адресами устройств, начиная с ближайшего, т.е свитч S2:

1) ___C:>ping 192.168.1.12___
2) ___C:>ping 192.168.1.11___
3) ___C:>ping 192.168.1.1___
...
Видим, что в таблицу коммутации свитча S2 добавился мас-адрес PC-B, которая как раз возникла в результатете работы ARP протокола.
  ...
  Internet Address      Physical Address      Type
  192.168.1.1           0060.5cdb.e5d0        dynamic
  192.168.1.11          0004.9ae7.9b01        dynamic
  192.168.1.12          0004.9aab.8cdd        dynamic
  ```
...
- ___В данной ситуации мы видим 3 пары как раз тех устройств на которые отсылались эхо-запросы, за исключением информациии о самом PC-B.___  
...
- ___Да, все устройства откликаются на эхо запросы.___

c) Подключимся через консоль к коммутатору S2, введим команду **show mac address-table**.
...
----    -----------       --------    -----
   ...
   1    0004.9ae7.9b01    DYNAMIC     Fa0/1
   1    000c.85b3.5d11    DYNAMIC     Fa0/18
   1    0060.5cdb.e5d0    DYNAMIC     Fa0/1
   1    0060.5c62.5799    DYNAMIC     Fa0/1
...
- ___Да, добавил еще  3 мас-адреса: это оба компьютера PC-A(192.168.1.1), PC-B(192.168.1.2) соответственно на интерфейсах Fa0/1 b Fa0/18, а также мас адрес интерфейса Vlan 1 коммутатора S1___ 
